...
# Semana 0 = sábado 13 de junio 2026
# Esa semana: JOSE ALLAICA queda LIBRE (índice 1 → semana 0 % 4 = 0... ajustamos offset)
# libre_index = (week + 1) % 4  → semana 0 da índice 1 = JOSE ALLAICA ✓
...
def get_week_number(ref: datetime = None) -> int:
    """Semanas transcurridas desde WEEK_ZERO."""
    if ref is None:
        ref = datetime.now(TZ)
    delta = ref - WEEK_ZERO
...
def get_free_index(week: int) -> int:
    """Índice del libre: semana 0 → JOSE (1), semana 1 → RYCHARD (2), etc."""
...
def get_schedule(week: int, data: dict) -> dict:
    """Devuelve on_duty y free considerando vacaciones activas."""
    now = datetime.now(TZ)
...
def build_reminder(week: int, data: dict) -> str:
    ...
    return (
        ...
        f"*🔧 DE TURNO (Sáb y Dom):*\n{turno_str}\n\n"
        f"*🟡 LIBRE este fin de semana:*\n{libre_str}\n\n"
        f"_Gosseal Machachi_ 💼"
    )


# ── Identificar miembro por usuario de Telegram ───────────────────────────────
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "`suspéndeme 7 días` → fuera 8 días (sáb+dom)\n"
        "`suspéndeme 15 días` → fuera 16 días (sáb+dom)\n\n"
...
async def cmd_calendario(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(8):
        ...
        lines.append(
            f"*Sem #{w}* — Sáb {sat.strftime('%d/%m/%Y')}{hoy}\n"
            f"  🔧 Turno: {turno}\n"
            f"  🟡 Libre: {libre}"
        )
...
async def cmd_estado(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    data = load_data()
    now = datetime.now(TZ)
    vac = data.get("vacations", {})
    activos = {k: v for k, v in vac.items()
               if datetime.fromisoformat(v) > now}

    if not activos:
        await update.message.reply_text("✅ Todos están activos, nadie suspendido.")
        return

    lines = ["🔴 *Personas suspendidas actualmente:*\n"]
...
# ── Handler de texto: suspéndeme ─────────────────────────────────────────────
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.lower().strip()

    # Detectar "suspéndeme" y variantes
    triggers = ["suspéndeme", "suspendeme", "suspéndame", "suspendame",
                "suspendame", "suspéndame"]
    if not any(t in text for t in triggers):
        return

    # Detectar días: 7 o 15
    ...
    dias_fuera = dias_pedidos + 1  # 7 → 8 días, 15 → 16 días

    member = find_member(update.effective_user)
    if not member:
        await update.message.reply_text(
            "⚠️ No puedo identificarte.\n"
            "Asegúrate de que tu nombre en Telegram coincida con el del equipo."
        )
        return
    ...
    await update.message.reply_text(
        ...
        f"📅 Te reincorporas automáticamente el *{retorno.strftime('%d/%m/%Y')}*.\n\n"
        f"Solo los *sábados y domingos* cuentan como turno.\n"
        f"¡Descansa! 🏖",
        parse_mode="Markdown"
    )


# ── Recordatorio automático cada viernes ─────────────────────────────────────
...
def main():
    ...
    # 3 recordatorios cada viernes (hora Ecuador)
    jq = app.job_queue
    for hour in [8, 13, 19]:
